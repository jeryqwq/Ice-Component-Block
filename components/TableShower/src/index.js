...
export default function TableShower({
  ...
  const [isAni,setIsAni]=useState(true);
   ...
   orderNumberStyles={...TableShower.defaultProps.orderNumberStyles,...orderNumberStyles};
  useEffect(() => {
  ...
  const transformStyle=function(){
    return `translateY(-${(itemTr.current?itemTr.current.clientHeight:0)*curIdx}px)`
  };
  const TableElItem=(
      data.map((item1,idx1)=><tr style={{background:idx1%2===0?oddRowBackgroundColor:evenRowBackgroundColor,}} key={idx1} ref={itemTr}>
        <td  width="40" className={"order-num"}><span  style={{...orderNumberStyles}}>{idx1+1}</span></td>
          {  columns.map((item2,idx2)=><td width={item2.width?item2.width:'50'} style={{...tbodyStyles}} key={idx2}>
          {
            typeof item1[columns[idx2]['key']] ==='function'?item1[columns[idx2]['key']](item1):item1[columns[idx2]['key']]}
          </td>)}
      </tr>)
  )
  return ( <div className = "TableShower-as2da2sf45as">
      <table>
          <thead>
             <tr style={{...theadStyles}} >
                <td  width="40"></td>
               {
                 columns.map((item,idx)=><td width={item.width?item.width:'50'} key={idx}>
                   {item.title}
                </td>)
               }
             </tr>
      ...
      <div className={'scroll-tab'} style={{height}} >
        <table  style={{transform:transformStyle()}} className={`${isAni?'ani':''}`}>
            <tbody>
                {TableElItem}
                {
...
TableShower.propTypes = {
  ...
  animationType: PropTypes.oneOf(["all", 'item']),
