...
import  './main.scss';
export default function ScrollText({text,isOnce,animationTime}) {
  ...
  const move=()=>{
   ...
   const interval= setInterval(() => {
       setLeft((val)=>{
          if(Math.abs(val)>width.current.textWidth){
              isOnePass=true;
  ...
  useLayoutEffect(()=>{//渲染完成重新计算宽度，自适应
    width.current={
      ...
      containerWidth:containerNode.current.offsetWidth
    }
  })
  useEffect(() => {
    const interval=move();
  ...
  return (
    <div className="scroll-text-wrap" ref={containerNode}>
      <span ref={textNode} className={'text'} style={{transform:`translateX(${left}px)`}}>{text}</span>
    </div>
  );
}
